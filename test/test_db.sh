...
until docker-compose $COMPOSE_FILE_PATH exec db mysql -u$DB_USER -p$DB_PASS -e "SELECT 1" > /dev/null 2>&1; do
...
error_message=$(docker-compose $COMPOSE_FILE_PATH exec -T db mysql -u$DB_USER -p$DB_PASS $DB_NAME < ./test/insert_test.sql 2>&1)
...
error_message=$(docker-compose $COMPOSE_FILE_PATH exec -T db mysql -u$DB_USER -p$DB_PASS $DB_NAME < ./test/select_test.sql 2>&1)
